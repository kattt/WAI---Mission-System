if(isServer) then {
	...
	/* AI CONFIG */
		...
		ai_share_distance			= 600;			// Distance from killed AI for AI to share your rough position

		ai_kills_gain				= false;			// add kill to bandit/human kill score
		ai_humanity_gain			= false;			// gain humanity for killing AI
	...
	/* WAI MISSIONS CONFIG */
		...
		wai_mission_spread			= 300;								// make missions spawn this far apart from one another and other markers
		...
		wai_mission_fuel			= [20,30];							// fuel inside mission spawned vehicles [min%,max%]
		wai_vehicle_damage			= [20,60];							// damages to spawn vehicles with [min%,max%]
		...
		wai_high_value_chance		= 3;								// chance in percent you find above mentioned item
		...
		crate_items_high_value		= ["ItemBriefcase_Base","ItemVault","ItemBriefcase_Base","ItemHotwireKit"];
